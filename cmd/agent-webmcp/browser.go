...
type OpenResult struct {
	...
	WebMCP   WebMCPStatus `json:"webmcp"`
...
func openURL(ctx context.Context, session, url, chromeBin string, headed bool, timeout time.Duration) (*OpenResult, error) {
	...
	// Stored custom tools for this host load BEFORE discovery, so a single
	// list captures native + overlay tools (previously: list, load, re-list
	// paid two full settle windows per open with custom tools).
	var custom []string
	if finalURL != "" {
		host := hostOfURL(finalURL)
		custom = append(custom, loadPacks(ctx, t.WebSocketDebuggerURL, host)...)
		recordOverlayTools(session, custom)
	}
	// WebMCP discovery (best-effort, fast): single pass over native+overlay.
	lctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	tools, _, _ := listWebMCP(lctx, t.WebSocketDebuggerURL)
	webmcp := WebMCPStatus{Experimental: true, Available: len(tools) > 0, ToolCount: len(tools)}
	frameCacheSaveAll(session, tools)
