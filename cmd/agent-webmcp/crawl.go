...
// harvestJS collects links + forms in one eval, output-bounded.
const harvestJS = `( () => {
  const links = {};
  ...
  return JSON.stringify({links, forms, inputs}).slice(0, 4000);
})()`
...
func crawlCmd(ctx context.Context, g *globals, rest []string) int {
	...
	if raw, herr := evalScript(ctx, t.WebSocketDebuggerURL, harvestJS, timeout); herr == nil {
		...
		if jerr := json.Unmarshal([]byte(raw), &h); jerr == nil {
			links, forms, inputs = len(h.Links), h.Forms, h.Inputs
		}
	}
	out := map[string]any{
		...
		"links_harvested": links, "forms": forms, "inputs": inputs,
