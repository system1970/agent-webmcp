...
(async () => {
  ...
  const tools = [
    {
      name: 'streamdown_chat_open',
      description: '[agent overlay — read-only] Open the Streamdown Ask AI panel via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    {
      name: 'streamdown_chat_close',
      description: '[agent overlay — read-only] Close the Ask AI panel via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use streamdown_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
