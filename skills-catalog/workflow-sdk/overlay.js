// agent-webmcp overlay for workflow-sdk.dev/docs: Ask AI panel verbs.
// Cloned 2026-09-15 from the vgpu skill (same panel family as eve),
// re-grounded live on workflow-sdk.dev/docs. Verbs: open, ask, read,
// clear, close.
//
// Grounded panel facts (workflow-sdk.dev/docs, headful):
...
// - Suggestions on a fresh panel; echo + answer + "Used N sources" line
//   per turn. The sources line is the completion marker — settle =
//   sources present AND text stable twice.
// - Controls: "Copy page", "Copy for LLM" (page-level extras),
//   "Copy chat" (disabled while empty), "Clear chat" (disabled while
//   empty = no-op), "Close chat". Clear wipes server-side (verified:
//   cleared answers stay gone across close+reopen).
// - Natives: NONE on this tenant (empty `list`) — panel only. Contrast
//   eve, whose search_docs + read_current_page end lookup jobs.
// - API-first lead (trace dump 2026-09-15): POST
//   https://workflow-sdk.dev/api/chat (200, 37KB, ~7.8s) carries the
//   turn — same first-party /api/chat path shape as eve.dev and
//   vgpu.sh. Unproven direct substitute.
// Install: agent-webmcp tools add overlay.js --for workflow-sdk.dev
(async () => {
  ...
  function transcriptScope() {
    const ta = textarea();
    if (ta) {
      let el = ta, best = document.body;
      for (let i = 0; i < 10 && el && el !== document.body; i++) {
        el = el.parentElement;
        if (el && (el.innerText || '').length > 200) best = el;
      }
      return best;
    }
  ...
  const tools = [
    {
      name: 'workflow_chat_open',
      description: '[agent overlay — read-only] Open the workflow Ask AI panel via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    {
      name: 'workflow_chat_close',
      description: '[agent overlay — read-only] Close the Ask AI panel via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use workflow_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
