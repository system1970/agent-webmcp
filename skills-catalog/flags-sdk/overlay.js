// agent-webmcp overlay for flags-sdk.dev/docs: Ask AI panel verbs.
// Cloned 2026-09-15 from the chat-sdk skill (same eve panel family),
// re-grounded live on flags-sdk.dev/docs. Verbs: open, ask, read,
// clear, close.
//
// Grounded panel facts (flags-sdk.dev/docs, headful):
...
//   leaves suggestions only). Contrast chat-sdk, where close+reopen
//   re-seats — clear semantics are per-tenant. Probe both states
//   before writing the manual.
// - Natives: NONE on this tenant (empty `list`) — panel only.
// - API-first lead (trace dump 2026-09-15): POST
//   https://flags-sdk.dev/api/chat (200, 20KB, ~3.7s) carries the
//   turn — same first-party /api/chat path shape as the family.
//   Unproven direct substitute.
// Install: agent-webmcp tools add overlay.js --for flags-sdk.dev
(async () => {
  ...
  function transcriptScope() {
    ...
    // answer inside flags-sdk.dev; clear/close reported false failures).
    const ta = textarea();
    if (ta) {
      let el = ta;
  ...
  const tools = [
    {
      name: 'flags_chat_open',
      description: '[agent overlay — read-only] Open the flags Ask AI panel via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    {
      name: 'flags_chat_close',
      description: '[agent overlay — read-only] Close the Ask AI panel via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use flags_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
