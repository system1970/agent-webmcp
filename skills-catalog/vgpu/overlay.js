// agent-webmcp overlay for vgpu.sh/docs: Ask AI panel verbs.
// Cloned 2026-09-15 from the eve skill (same panel family), re-grounded
// live on vgpu.sh/docs. Verbs: open, ask, read, clear, close.
//
// Grounded panel facts (vgpu.sh/docs, headful):
...
//   https://vgpu.sh/api/chat (200, 35KB, ~4.6s) carries the turn —
//   same first-party /api/chat path shape as eve.dev. Unproven direct
//   substitute.
// Install: agent-webmcp tools add overlay.js --for vgpu.sh
(async () => {
  ...
  const tools = [
    {
      name: 'vgpu_chat_open',
      description: '[agent overlay — read-only] Open the vgpu Ask AI panel via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    },
    {
      ...
      description: '[agent overlay — read-only] Close the Ask AI panel via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use vgpu_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
