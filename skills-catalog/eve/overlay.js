// agent-webmcp overlay for eve.dev/docs: Ask AI dock verbs.
// Built 2026-09-15 from raw scan/act turns (headful Chrome). Verbs:
// open, ask, read, clear, close. Custom panel family (not Mintlify).
//
// Grounded panel facts (eve.dev/docs, headful):
// - Trigger: BUTTON "Ask AI" (visible). Click opens the dock.
...
// - Suggestions: 4 question chips on a fresh panel; echo + answer +
//   "Used N sources" line per turn. The sources line is the completion
//   marker — settle = sources present AND text stable twice.
// - Controls: "Copy chat" (disabled while empty), "Expand chat",
//   "Clear chat" (disabled while empty = no-op), "Close chat".
//   Clear is a LOCAL wipe (sheet empties; close+reopen re-seats the
//   last server turn — verified 2026-09-15 on chat-sdk; eve re-check
//   pending). Use it for a clean sheet between tasks, not as a
//   privacy boundary.
// - Natives (no overlay needed): search_docs (rich, highlighted —
//   verified), read_current_page (full page Markdown — verified).
// - API-first lead (trace dump 2026-09-15): POST
//   https://eve.dev/api/chat (200, 6KB, ~5.3s) carries the turn —
//   candidate direct substitute, unproven.
// Install: agent-webmcp tools add overlay.js --for eve.dev
(async () => {
  ...
  function askButton() {
    return [...document.querySelectorAll('button')]
      .find((b) => /^ask ai$/i.test(((b.getAttribute('aria-label') || b.innerText) || '').trim())) || null;
  }
  function textarea() {
    return [...document.querySelectorAll('textarea')]
      .find((t) => /what would you like to know/i.test(t.placeholder || '')) || null;
  }
  function submitBtn() {
    const ta = textarea();
    let scope = ta;
  ...
  async function ensureOpen() {
    ...
    throw new Error('Ask AI clicked but dock did not open');
  }
  function transcriptScope() {
    // Scope to the dock's ASIDE: document-wide reads hit stale page
    // text and ghosts (2026-09-15: body read showed a dead chat-sdk
    // answer inside eve.dev; clear/close reported false failures).
    const ta = textarea();
    if (ta) {
      let el = ta;
  ...
  async function send(message) {
    ...
    await ensureOpen();
    const ta = textarea();
    if (!ta) throw new Error('dock open but composer missing');
    setText(ta, message);
    const btn = submitBtn();
  ...
  const tools = [
    {
      name: 'eve_chat_open',
      description: '[agent overlay — read-only] Open the eve Ask AI dock via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    {
      name: 'eve_chat_read',
      description: '[agent overlay — read-only] Read the latest dock transcript without sending anything. Recovery poll when ask lost its return: returns {question (last asked), answer, sourcesUsed}. Never mutates the panel.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
        const r = readNow();
        let ans = r.text || '';
    ...
    {
      name: 'eve_chat_clear',
      description: '[agent overlay] Wipe the dock thread via its "Clear chat" control (disabled while empty = no-op with already:true). LOCAL wipe — close+reopen re-seats the last server turn (chat-sdk proven; eve re-check pending). Use for a clean sheet between tasks, not as a privacy boundary. Acts-grade: call between tasks, never mid-task.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
        await ensureOpen();
        const btn = clearBtn();
    ...
    {
      name: 'eve_chat_close',
      description: '[agent overlay — read-only] Close the Ask AI dock via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use eve_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
        ...
        return J({ closed: false, note: 'close clicked but dock still open' });
      },
    },
  ];
