// agent-webmcp overlay for chat-sdk.dev/docs: Ask AI panel verbs.
// Cloned 2026-09-15 from the eve skill (same panel family),
// re-grounded live on chat-sdk.dev/docs. Verbs: open, ask, read,
// clear, close. Custom panel family (not Mintlify).
//
// Grounded panel facts (chat-sdk.dev/docs, headful):
...
//   last server turn — verified 2026-09-15). Use it for a clean sheet
//   between tasks, not as a privacy boundary.
// - Natives: NONE on this tenant (empty `list`) — panel only.
// - API-first lead (trace dump 2026-09-15): POST
//   https://chat-sdk.dev/api/chat (200, 44KB, ~10s) carries the turn —
//   same first-party /api/chat path shape as the family. Unproven
//   direct substitute.
// Install: agent-webmcp tools add overlay.js --for chat-sdk.dev
(async () => {
  ...
  function transcriptScope() {
    ...
    // answer inside chat-sdk.dev; clear/close reported false failures).
    const ta = textarea();
    if (ta) {
      let el = ta;
  ...
  const tools = [
    {
      name: 'chat_chat_open',
      description: '[agent overlay — read-only] Open the chat Ask AI dock via its trigger. Open = composer sized. No-op with already:true when open.',
    ...
    {
      name: 'chat_chat_clear',
      description: '[agent overlay] Wipe the dock thread via its "Clear chat" control (disabled while empty = no-op with already:true). Clears server-side — cleared turns stay gone across close+reopen. Use between unrelated tasks sharing one session. Acts-grade: call between tasks, never mid-task.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
        await ensureOpen();
        const btn = clearBtn();
    ...
    {
      name: 'chat_chat_close',
      description: '[agent overlay — read-only] Close the Ask AI dock via its "Close chat" control. No-op with already:true when closed. Note: closing keeps history — use chat_chat_clear to wipe the thread.',
      inputSchema: { type: 'object', properties: {} },
      execute: async () => {
