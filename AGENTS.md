...
| Jev experiments, traces, custom tool sources (`lab/`) | `lab/AGENTS.md` |
| Decision policy + thresholds | `lab/OBSERVATIONS.md` (8 findings from real-site runs) |
...
There are no Go tests yet. Live-browser checks are the suite: `lab/TASKS.md`
battery (read-only goals, traces to `lab/traces/*.jsonl`).
...
- Keys never enter the repo (`.env.local` stays local; re-enter per `lab/HANDOFF.md`).
- The task battery is read-only: no accounts, no purchases, no form submissions
  with real data (`lab/TASKS.md`).
